...
public class MembershipPlanRepository {
    ...
    public Optional<MembershipPlan> findById(String planId) {
        MembershipPlan plan = membershipPlanTable.getItem(r -> r.key(k -> k.partitionValue(planId)));
        return Optional.ofNullable(plan);
    }

    public List<MembershipPlan> findAll() {
        List<MembershipPlan> plans = new ArrayList<>();
        membershipPlanTable.scan().items().forEach(plans::add);
        return plans;
    }

    public void deleteById(String planId) {
        membershipPlanTable.deleteItem(r -> r.key(k -> k.partitionValue(planId)));
    }
}